...
#
# Then the call of `print_array(list)` should produce the following output:
...
def strech(array)
  ...
  for i in 0...array.length
    if array[i] % 2 == 0
      repeat_number = array[i] / 2
      for i in 1..2
        strech_array << repeat_number
      end
    else
      single_number = array[i] / 2
      strech_array << single_number + 1
      strech_array << single_number
    end
  end
  return strech_array
end
...
def numUnique(sorted_array)
  ...
  for i in 0...sorted_array.length
    counter_hash[sorted_array[i]] = "unique"
  end
  return counter_hash.length
end

#NOTE: I doubt this was the 'offical' answer you were looking for but, it does work.
#Technically, I just need to use the keys of the hash to keep track of the unique numbers.
#If a new key 'coming in' into the hash already matches another key it will just replace
#the orginal one. If the incoming key does not match an existing key, the incoming key
